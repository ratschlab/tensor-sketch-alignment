...
if __name__ == '__main__':
    ...
    header = f'>Base sequence with K-{K}'
    ...
    # Generate graph
    build_command = f"{METAGRAPH_PATH} build -k {K} -o {os.path.join(DATASET_DIR, INPUT_SEQ.split('.')[0])} {graph_seq_path}"
    subprocess.run(build_command.split())
    print("[LOG] Saved .dbg file from generated sequence")
    ...
    for i in range(NUM_QUERY_SEQS):
        ...
        header = f">Query string {i}, mutation rate {MUTATION_RATE}"
